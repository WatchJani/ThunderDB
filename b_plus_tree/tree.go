...
// 	rangeFn, nextFn := CommandRange[V](command)
...
// func CommandRange[V any](command string) (func(int, [][]byte, *Node[V]) bool, func(*Node[V]) *Node[V]) {
// 	switch command { // is range "<=" in relation to the source
// 	case "<=":
// 		return func(index int, destination [][]byte, node *Node[V]) bool {
// 				return node.items[index].key > destination
// 			}, func(n *Node[V]) *Node[V] {
// 				return n.nextNodeL
// 			}
// 	case ">=":
// 		return func(index int, destination [][], node *Node[K, V]) bool {
// 				return node.items[index].key < destination
